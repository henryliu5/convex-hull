...
func hull(points[][2]float32, min_pt [2]float32, max_pt [2]float32){
	fmt.Println(points)
	if (len(points) == 0){
		return
	}
	...
	hull(L1, furthest_pt,max_pt)
    hull(L2, min_pt, furthest_pt)
}
...
func quickhull(points [][2]float32){
	...
	hull(above_pts, min_pt, max_pt)
	hull(below_pts, max_pt, min_pt)
}
